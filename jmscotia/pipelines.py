...
class JmscotiaPipeline:

    # Database setup
    conn = sqlite3.connect('jmscotia.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `jmscotia`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM jmscotia WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO jmscotia (date, title, link, content)"
